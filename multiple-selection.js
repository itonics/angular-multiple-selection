...
angular.module('multipleSelection', [])
  .factory('multipleSelectionService', function() {
    return {
      allSelectables: [],
      selectedItems: [],
  ...
  .directive('multipleSelectionItem', ['multipleSelectionService', function(multipleSelectionService) {
    return {
      scope: false,
      restrict: 'A',
      require: "^multipleSelectionZone",
      controller: function($scope) {
        $scope.msService = multipleSelectionService;
      },
      link: function(scope, element, iAttrs, multiZoneCtrl) {
        ...
        if (scope.isSelectable) {
          scope.msService.allSelectables.push(element);
        }
        ...
        element.on('mouseup touchend', function(event) {
          if (event.which == 1) {
            if (!scope.msService.continuousSelection) {
              if (element.scope().isSelected) {
                ...
                if (!event.ctrlKey) {
                  ...
                  var childs = scope.msService.allSelectables;
                  for (var i = 0; i < childs.length; i++) {
        ...
        element.on('click mousedown touchend', 'a', function(event) {
          event.stopPropagation();
        });
      }
    };
  }])
  .directive('multipleSelectionZone', ['$document', '$timeout', 'multipleSelectionService', function($document, $timeout, multipleSelectionService) {
    return {
      restrict: 'A',
      scope: {
        selectedItems: "="
      ...
      controller: function($scope) {
        ...
        this.updateSelection = $scope.updateSelection = function() {
          var items = $scope.msService.allSelectables;
          var selectedItems = [];
          for (var i = 0; i < items.length; i++) {
            if (items[i].scope().isSelected) {
              selectedItems.push(items[i]);
            }
          }

          $timeout(function() {
            $scope.selectedItems = $scope.msService.selectedItems = selectedItems;
      ...
      link: function(scope, element, attrs, controller) {
        ...
        if (typeof attrs["continuousSelection"] !== 'undefined') {
          scope.continuousSelection = scope.msService.continuousSelection = true;
        }
        ...
        function mouseup(event) {
          ...
          for (var i = 0; i < childs.length; i++) {
            if (childs[i].scope().isSelecting === true) {
              ...
              /*if (!scope.msService.continuousSelection && checkElementHitting(transformBox(childs[i].prop('offsetLeft'), childs[i].prop('offsetTop'), childs[i].prop('offsetLeft') + childs[i].prop('offsetWidth'), childs[i].prop('offsetTop') + childs[i].prop('offsetHeight')), transformBox(event.pageX, event.pageY, event.pageX, event.pageY))) {
